 ...
 */
const MAX_IMAGE_BYTES = 3 * 1024 * 1024; // 3 МБ — защита от огромных картинок
...
async function toDataUri(url) {
  ...
  try {
    ...
    if (ct && !ct.startsWith('image/')) return null; // только картинки

    const cl = parseInt(res.headers.get('content-length') || '0', 10);
    if (cl > MAX_IMAGE_BYTES) return null; // слишком большой файл — пропускаем

    // Читаем побайтово с потолком, чтобы не загрузить память если content-length не указан
    const chunks = [];
    let total = 0;
...
async function fetchPostPreview({ channelUsername, postId }) {
  ...
  // Если конвертация упала — оставляем прямой URL как фолбэк.
  const photos = await Promise.all(
    photoUrls.slice(0, 2).map(async url => (await toDataUri(url)) || url)
  );
...
/**
 * Находит id нескольких последних публичных постов в канале через t.me/s/<channel> —
 * публичную "ленту" канала (та же страница, что видна без логина в браузере).
 *
 * Возвращает { postIds } — массив id, самый свежий пост первым (может быть короче
 * запрошенного limit, если в канале меньше постов) — либо { postIds: [], reason }
 * с пояснением, почему постов не нашлось; это попадает в лог консоли и в сообщение
 * бота в чат, чтобы не было тихого "пустого" фолбэка без объяснений.
 */
async function fetchLatestPostIds(channelUsername, limit = 5) {
  ...
  if (dataPostAttrs.length === 0) {
    ...
    return { postIds: [], reason };
  }

  // лента идёт от старых к новым — переворачиваем, чтобы самый свежий пост был первым,
  // и берём не больше limit штук
  const postIds = dataPostAttrs
    .slice(-limit)
    .reverse()
    .map((entry) => entry.split('/')[1])
    .filter(Boolean);

  return { postIds, reason: postIds.length ? null : 'не удалось распарсить id постов' };
}
...
/**
 * Главная функция: ссылка -> массив объектов с данными для оверлея.
 *
 * - Ссылка на конкретный пост (t.me/channel/123) -> массив из одного поста.
 * - Ссылка просто на канал (t.me/channel) -> массив из нескольких (по умолчанию
 *   до 5) последних постов канала, самый свежий первым — чтобы оверлей мог
 *   показать их по очереди, а не только самый последний.
 *
 * Поля канала (channelUsername, channelTitle, subscriberCount, avatarUrl,
 * avatarDataUri) одинаковы у всех элементов массива — оверлею они нужны
 * только один раз, но так проще (каждый пост самодостаточен).
 */
async function getTelegramPreviews(rawUrl, { limit = 5 } = {}) {
  ...
  if (!parsed.isChannelOnly) {
    return [await fetchPostPreview(parsed)];
  }

  const { postIds, reason } = await fetchLatestPostIds(parsed.channelUsername, limit);
  if (postIds.length === 0) {
    console.warn(`[telegram] Не нашёл посты @${parsed.channelUsername}: ${reason}`);
  ...
  // Тянем превью постов параллельно, но терпимо к отдельным сбоям —
  // если один пост не спарсился, остальные всё равно покажем.
  const settled = await Promise.allSettled(
    postIds.map((postId) => fetchPostPreview({ channelUsername: parsed.channelUsername, postId }))
  );
  ...
  if (previews.length === 0) {
    console.warn(`[telegram] Все посты @${parsed.channelUsername} не спарсились`);
    const channelPreview = await fetchChannelPreview(parsed);
    return [{ ...channelPreview, isFallback: true, fallbackReason: 'не удалось загрузить ни один из последних постов' }];
  }
 ...
 * Возвращает только самый свежий пост (первый элемент массива).
 */
async function getTelegramPreview(rawUrl) {
  const [first] = await getTelegramPreviews(rawUrl, { limit: 1 });
  return first;
}

module.exports = { getTelegramPreview, getTelegramPreviews, parseTelegramLink };
